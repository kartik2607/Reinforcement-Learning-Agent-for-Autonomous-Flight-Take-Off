...
def read_txt(nam):
    ...
    new_path = os.path.join(cur_path,"Pos_and_angles",nam,"positions.txt")
    raw_pos = open(new_path, "r").read()
    Pos_vec = eval(raw_pos)

    new_path = os.path.join(cur_path,"Pos_and_angles",nam,"angles.txt")
...
def animate_plane(nam):
    Pos_vec, theta_vec = read_txt(nam)
    y_vec = Pos_vec[1]
    x_vec = Pos_vec[0]
    ...
    SCREEN_WIDTH = 800
    SCREEN_HEIGHT = 800
    SCREEN_TITLE = "Move Sprite by Angle Example"+nam
